...
<head>
  ...
  <script>
	...
	function toggleBizForm(isBiz) {
	  ...
	  document.getElementById('bizRow2').style.display = isBiz ? '' : 'none';
	}
	...
	// 입력 여부 확인
	  ...
	  // 형식 검사
	  if (!regMid.test(mid)) {
	    alert("아이디는 5~20자의 영문, 숫자, 특수문자(_,-)만 사용 가능합니다.");
	    myform.mid.focus();
	    return;
	  }
	  if (!regPwd.test(pwd)) {
  ...
  <form name="myform" method="post" enctype="multipart/form-data">
    <table class="join-table">
      ...
      <tr>
        ...
        <td>
          <div class="input-group">
            ...
            <button type="button" onclick="emailCertification()" id="certificationBtn" class="btn btn-info btn-sm ms-2">인증번호받기</button>
          </div>
      ...
      <tr id="bizRow1" style="display: none;">
        <th class="bg-secondary-subtle">사업자번호</th>
        <td><input type="text" name="businessNo" class="form-control" placeholder="숫자 10자리 입력" /></td>
      </tr>
      <tr id="bizRow2" style="display: none;">
        <th class="bg-secondary-subtle">숙소명</th>
        <td><input type="text" name="storeName" class="form-control" placeholder="숙소명을 입력하세요" /></td>
      </tr>
    </table>
    ...
    <!-- 선택입력사항 -->
    <div class="form-check mb-2" id="viewCheckBtn">
      <label for="viewBtn" class="form-check-label" style="color: #2e7d32; font-weight: bold; font-size: 1rem;"><i class="bi bi-sliders me-1"></i> 선택입력항목</label>
    </div>
    <div id="addContent">
      <table class="join-table">
        <tr>
