...
public class ChattingServer {
  ...
  public void onMessage(String message, @PathParam("username") String username) throws IOException {
    ...
    if (parts.length == 2) {
      ...
      if (targetSession != null) targetSession.getBasicRemote().sendText(username + ": " + msg);
    }
  }
  
  // 접속한 유저리스트 모두 담기(구분을 위해서 'USER_LIST:'뒤에 유저명들을 저장시킨다. 예) USER_LIST:admin,hkd1234,kms1234)
